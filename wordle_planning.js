...
var difficultWords = ["wound", "joker",]
...
function main()
{
    ...
    {
        let printText = "";
        let keyWord = wordList[i];//[Math.floor(Math.random() * difficultWords.length)];
        let solved = false;
    ...
    output.innerHTML += "Could not solve " + unsolvedWords.length + " words:<br><b>" + unsolvedWords + "</b>";
    ...
    for (var wordIndex = 0; wordIndex < wordList.length; wordIndex++)
    {
        ...
        for (var letterIndex = 0; letterIndex < word.length; letterIndex++)
        {
            ...
            if (getGreyLetters().includes(word[letterIndex]))
                score = -999999999;
	        // Otherwise, add two values to the word's score:
	        //   - The letter's frequency overall in the list of words
	        //   - The letter's frequency in that particular index in the word
            //
            else
                score += letterTotalFrequency(word[letterIndex]);
